...
from models import EnergyLSTM, EnergyLSTMFrozen


def train_frozen_backbone(source_building, target_building,
    ...
    """Frozen-backbone transfer: LSTM encoder locked, only head fine-tuned.

    The pre-trained LSTM weights are loaded and immediately frozen.
    Only the MLP head (~8 K params) is updated during training on the target
    building data.  This prevents catastrophic forgetting entirely and is most
    effective in the low-data regime.
    ...
    print(f"\n{'='*70}")
    print(f"  FROZEN BACKBONE: Only head fine-tuned on limited data")
    ...
    # Build frozen-backbone model and copy weights from source
    frozen_model = EnergyLSTMFrozen(
        ...
        dropout=0.2,
        learning_rate=1e-3,  # Higher LR fine — only head is updated
    )
    # Copy all weights (LSTM + head); LSTM params are already frozen in __init__
    frozen_model.load_state_dict(source_model.state_dict())
    ...
    print(f"   Input size    : {frozen_model.hparams.input_size}")
    print(f"   Hidden size   : {frozen_model.hparams.hidden_size}")
    print(f"   Num layers    : {frozen_model.hparams.num_layers}")
    print(f"   Total params  : {total_params:,}")
    print(f"   Trainable     : {trainable_params:,}  (head only)")

    exp_dir = os.path.join(project_root, 'models', 'experiments', experiment_name)
    os.makedirs(exp_dir, exist_ok=True)
    checkpoint_callback = ModelCheckpoint(
        dirpath=exp_dir,
        filename=f'frozen_{source_building[:15]}_{target_building[:15]}_{{epoch:02d}}_{{val_loss:.4f}}',
        monitor='val_loss',
        mode='min',
        save_top_k=1,
    )
    ...
    print("\nStarting frozen-backbone fine-tuning...")
    trainer.fit(frozen_model, train_loader, val_loader)

    print("\nTesting...")
    results = trainer.test(frozen_model, test_loader)
...
if __name__ == '__main__':
    ...
    model, results = train_frozen_backbone(
