...
PRIME, using a performance-weighted blend of five Eagle Education buildings (same site, same building type), failed catastrophically in all low-data regimes (643.5~kWh MAE at 8 weeks vs.\ 90.5~kWh for Scratch = $-$611\%), revealing that data-completeness-based source weighting within a homogeneous source pool provides no improvement over training from scratch. This negative result provides important insights for source selection methodologies.
...
\begin{enumerate}
    ...
    \item \textbf{Adapter Training:} The Adapter strategy was designed but not fully trained in all experiments. Results for Adapter are incomplete and marked as NaN in several experiment outputs.
...
\begin{enumerate}
    ...
    \item \textbf{PRIME Negative Result:} A well-documented negative result showing that performance-weighted source blending fails in low-data regimes when source pools lack distributional diversity, revealing that in-domain validation error does not predict cross-domain transfer utility.
...
\textbf{Note:} Adapter training was not completed for all experiments in this project. Results for Adapter are marked as incomplete.
...
\textbf{Key Takeaway:} Single-source Full Fine-Tuning is beneficial in most cases but catastrophically fragile for hard targets. Frozen Backbone provides a safer default, especially excelling at Eagle/Brooke where it is the only viable strategy.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig07_8week_snapshot}
\caption{8-week MAE comparison across all six building pairs for Scratch, Full Fine-Tuning, and Frozen Backbone. The Eagle/Brooke Full FT bar is clipped (actual 335 kWh) for visual clarity. Frozen Backbone achieves the lowest MAE for Eagle/Brooke (50.9 kWh), while Full FT dominates for Lamb (+29\%) and Office (+24.7\%).}
\label{fig:8week_bar}
\end{figure}
...
In multi-source transfer, not all sources are equally useful. PRIME selects sources based on their validation-set performance on their own data (data completeness), weighting higher-quality sources more heavily during initialisation.

\textbf{PRIME Method:}
\begin{enumerate}
    \item Rank candidate source buildings by a composite data-quality score (completeness and continuity of meter readings).
...
PRIME's scoring algorithm ranked available buildings by data completeness. The top-5 selected sources were:

\begin{table}[H]
\centering
\caption{PRIME Source Pool: Top-5 Selected Buildings (Eagle/Brooke target)}
\small
\begin{tabular}{clccc}
\toprule
\textbf{Rank} & \textbf{Building} & \textbf{Site} & \textbf{Type} & \textbf{Completeness (\%)} \\
\midrule
1 & Eagle/Samantha & Eagle & Education & 99.93 \\
2 & Eagle/Will & Eagle & Education & 99.54 \\
3 & Eagle/Luther & Eagle & Education & 99.53 \\
4 & Eagle/Teresa & Eagle & Education & 99.61 \\
5 & Eagle/Sherrill & Eagle & Education & 99.48 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Critical observation:} All five selected sources are on the same Eagle site and are the same Education building type as the target (Eagle/Brooke). The PRIME scoring mechanism, optimising for data completeness, produced a \textit{completely homogeneous} source pool: same-site, same-type as the target.

\subsection{Results: Catastrophic Failure in Low-Data Regime}

\begin{table}[H]
\centering
\caption{PRIME vs.\ Scratch (Eagle/Brooke target)}
\small
\begin{tabular}{lccc}
\toprule
\textbf{Weeks} & \textbf{Scratch MAE (kWh)} & \textbf{PRIME MAE (kWh)} & \textbf{PRIME vs Scratch (\%)} \\
\midrule
1 & 867.21 & 918.43 & $-$5.9\% (PRIME worse) \\
2 & 671.91 & 920.06 & $-$36.9\% (PRIME worse) \\
4 & 66.55 & 854.78 & $-$1184\% (\textbf{collapse}) \\
8 & 90.51 & 643.54 & $-$611\% (\textbf{catastrophic}) \\
16 & 56.09 & 112.99 & $-$101\% \\
32 & 93.01 & 87.91 & +5.5\% (crossover) \\
64 & 59.65 & 46.72 & +21.6\% \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key finding:} PRIME fine-tuning collapses catastrophically for Eagle/Brooke:
\begin{itemize}
    \item At \textbf{8 weeks}: PRIME = 643.5~kWh vs.\ Scratch = 90.5~kWh → \textbf{$-$611\% benefit}
    \item At \textbf{4 weeks}: PRIME = 854.8~kWh vs.\ Scratch = 66.6~kWh → \textbf{$-$1184\% benefit}
    \item First meaningful PRIME advantage only at 32+ weeks (Scratch = 93.0, PRIME = 87.9, +5.5\%)
\end{itemize}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig14_prime_comparison}
\caption{PRIME vs.\ Scratch for Eagle/Brooke. PRIME fine-tuning collapses catastrophically in the 1--16 week regime ($-$611\% at 8 weeks). The crossover occurs only at 32+ weeks, by which point Scratch has already recovered. The homogeneous source pool (all Eagle Education buildings) means PRIME sources are themselves distribution-mismatched for fine-tuning onto Eagle/Brooke.}
\label{fig:prime_comparison}
\end{figure}

\subsection{Analysis: Why PRIME Failed}

\textbf{Root cause: Homogeneous source pool + Fine-Tuning collapse.}

PRIME's scoring function, based on data completeness, selected five Eagle Education buildings---all co-located with Eagle/Brooke and functionally identical. This is the most homogeneous source pool possible: same site, same building type, same institutional operations.

Despite this apparent ``ideal'' match, fine-tuning on Eagle/Brooke with only 1--16 weeks of target data collapses regardless of initialisation quality. This is the same collapse mechanism observed across all FT approaches (single-source, multi-source, ensemble, N-source). A homogeneous initialisation does not fix the fundamental problem: unrestricted parameter updates on a hard distribution-mismatched target corrupt LSTM weights.

\textbf{Secondary issue---Data completeness $\neq$ Transfer utility:} PRIME ranks sources by data completeness, not by model-transfer quality. A building with 99.9\% complete data is not necessarily a better transfer source for fine-tuning. All five selected sources (Samantha, Will, Luther, Teresa, Sherrill) are Eagle Education buildings that were themselves poor single-source FT sources in earlier experiments.

\textbf{What would a better source selection scheme need?}
\begin{itemize}
    \item Select sources based on \textit{frozen-backbone} validation loss on the target (use a small seed dataset to estimate transfer quality before committing to fine-tuning).
    \item Or use domain-similarity metrics (e.g., feature-space distance) to assess \textit{compatibility} rather than source \textit{quality}.
\end{itemize}

\textbf{Positive result at 32+ weeks:} Once sufficient target data is available ($\geq$32 weeks), the same PRIME initialisation enables useful fine-tuning: PRIME reaches 87.9~kWh at 32w vs.\ Scratch 93.0~kWh (+5.5\%), and 46.7~kWh at 64w vs.\ Scratch 59.6~kWh (+21.6\%). With ample data, the distribution mismatch problem is overcome and the homogeneous same-site initialisation provides a genuinely useful head start.

\textbf{Practical lesson:} In the data-scarce regime ($\leq$16 weeks), PRIME-style performance-weighted FT provides no advantage and catastrophically fails for hard targets. Frozen Backbone with any reasonable source remains the only robust strategy.
...
\midrule
PRIME & 643.5~kWh at 8w ($-$611\% vs.\ Scratch 90.5~kWh). Source pool was homogeneous (5$\times$ Eagle Education); completeness scoring $\neq$ transfer utility. & Use Frozen Backbone; PRIME only helps at 32+ weeks \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Adapter Strategy:} Incomplete evaluation due to training failures. Preliminary results suggest performance between Frozen Backbone and Full Fine-Tuning, but instability issues require further investigation.

\subsection{RQ3: Domain Distance Impact}
...
\subsection{Adapter Training (Internal Validity)}

\textbf{Limitation:} Adapter strategy not fully trained in all experiments. Results for Adapter incomplete, marked as NaN in several outputs.

\textbf{Threat:} Adapter may be more effective than reported. Incomplete evaluation prevents fair comparison.

\textbf{Mitigation:} Debug and complete Adapter training. Tune adapter hyperparameters (bottleneck dimension, learning rate). Include Adapter in final strategy comparison.
